...
type GetEnrollLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetEnrollLogic(ctx context.Context, svcCtx *svc.ServiceContext) GetEnrollLogic {
	return GetEnrollLogic{
...
func (l *GetEnrollLogic) GetEnroll() (*types.UsersInfo, error) {
	// 拿部门id
	...
	users, err := l.svcCtx.RegModel.FindFinalStatus(filed.PASS, depId)
	if err != nil {
	...
	for i, u := range usersRsp.Users {
		if users[i].Uid == u.Uid {
			...
			pass := users[i]
			if pass.SecondStatus == filed.PASS {
				m.Eva = pass.SecondEva.String
				m.Mark = int(pass.SecondMark)
			} else {
				m.Eva = pass.FirstEva.String
				m.Mark = int(pass.FirstMark)
			}
			rsp = append(rsp, m)
		}
	}
